...
function CategoryItem({name, href, backgroundColor, color}) {
  ...
  return (
    <div>
      <Link to={href} className='rounded-full'>  {/* Corrected from `<link>` to `<Link>` */}
        <div className='uppercasw px-6 py-2 text-center rounded-full' style={style}  >{name}</div>
      </Link>
    </div>
  );
}
function CategoryList() {
  return (
    <div className='flex flex-wrap items-center justify-center gap-8'>
      ...
      <CategoryItem  name="breakfast" href="/categories/breakfast" backgroundColor="#efedfa" color="3c3aBf"/>
